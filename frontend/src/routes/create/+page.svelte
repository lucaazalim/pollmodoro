<script lang="ts">
  ...
  const form = superForm(data?.form ?? defaults({
    title: '',
    description: '',
    options: ['', ''],
    pollType: 'multiple_choice' as const,
    allowMultipleOptions: false
  }, zodClient(createPollSchema)), {
    SPA: true,
    validators: zodClient(createPollSchema),
    async onUpdate({ form }) {
      if (form.valid) {
        try {
...
<div class="max-w-2xl mx-auto p-6">
  <h1 class="text-3xl font-bold mb-8">Create a New Poll</h1>
  ...
  <form use:enhance class="space-y-6">
    ...
    <div class="space-y-3">
      <div>
        ...
        <p class="text-sm text-muted-foreground mt-1">Add 2-10 options for people to choose from</p>
      </div>
    ...
    <Form.Field {form} name="allowMultipleOptions" class="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
      <Form.Control>
        {#snippet children({ props })}
          <input
            {...props}
